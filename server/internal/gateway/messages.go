...
type inbound struct {
	...
	ShellCommand          *session.ShellCommand `json:"shell_command"`           // on shell_command_put: the pre-configured shell command to add/update (updated_at inside)
	...
	ShellCommandsDigest   string                `json:"shell_commands_digest"`   // on `hello`: the app's digest of its cached shell-command catalogue (same fast path)
...
// msgShellCommands advertises the app-managed shell-command catalogue — the closed
// set of command lines a shell token may run on the target host. Each entry is the
// full session.ShellCommand so the app's editor can round-trip every field; the
// structs marshal directly, so their JSON tags are the wire shape.
func msgShellCommands(cmds []*session.ShellCommand) map[string]any {
	if cmds == nil {
		cmds = []*session.ShellCommand{}
	}
	return map[string]any{"type": "shell_commands", "shell_commands": cmds}
}
...
//
// The rotation mints a fresh `session_id` (the old one is retired onto the
