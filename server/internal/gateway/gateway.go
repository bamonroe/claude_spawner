...
func New(cfg *config.Config, store *session.Store, hosts *session.HostStore, ids *session.IdentityStore, tokens *session.SpokenTokenStore, shellCmds *session.ShellCommandStore, sshPool *session.SSHPool, driver *session.Driver, tmuxMgr *tmux.Manager, stt transcribe.Transcriber, ttsClient *tts.Client) *Server {
	...
	// was unreachable.
	go s.purgeRetryLoop()
	// Push the session list to every client whenever it changes, so a spawn/rename/
	// delete on one device lands on the others without a refresh.
	s.watchSessionList()
	return s
}
...
// hub's attribution point: each frame is attributed to the job's session (via the
// shared [attribute] resolver, authoritatively — so it tracks a compress rotation),
// letting a device viewing a different session route it to the right chat log
// instead of misfiling it under whatever it currently shows. The id is read
// lock-free (j.sid) because one bindJob sink call runs before j.mu is held. We
// overwrite any session_id a builder may have set — the hub is authoritative — but
// leave the display `name` the builder chose.
func (c *conn) jobSink(j *sessionJob) func(any) bool {
	return func(v any) bool {
		if m, ok := v.(map[string]any); ok {
			attribute(m, j.sid(), "", true)
		}
		c.wmu.Lock()
		closed := c.closed
		c.wmu.Unlock()
...
var wireHandlers = map[string]func(c *conn, in inbound){
	...
	"history": func(c *conn, in inbound) {
		c.startHistory(in.SessionID, in.Name, in.Before, in.Limit, in.HaveHash, in.HavePrefix, in.HavePrefixCount, in.Background)
	},
