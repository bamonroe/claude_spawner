...
impl WakeWordModel {
    ...
    /// Like [`predict`](Self::predict) but scores **every** 16-embedding window across the clip
    /// (sliding by `hop` embeddings) and returns the **peak** score per classifier. The mel and
    /// embedding networks — the expensive part — run exactly **once** over the whole clip; only the
    /// tiny classifier is re-run per window. This catches a wake word wherever it falls in a longer
    /// utterance (e.g. "bump bump &lt;command&gt;") without the N× embedding recompute that calling
    /// [`predict`](Self::predict) on overlapping sub-slices would incur. `hop` is clamped to >= 1.
    pub fn predict_peak(
        &mut self,
        audio_chunk: &[i16],
        hop: usize,
        ...
        let embeddings = self.compute_embeddings(audio_chunk)?;
        if embeddings.len() < MIN_EMBEDDINGS {
            return Ok(self.zero_scores());
        }

        // Window start offsets: slide by `hop`, always including the final flush-right window so a
        // token at the very tail is never missed.
        let last_start = embeddings.len() - MIN_EMBEDDINGS;
        let mut starts: Vec<usize> = (0..=last_start).step_by(hop).collect();
        if *starts.last().unwrap() != last_start {
            starts.push(last_start);
        }

        let mut peak = self.zero_scores();
        for start in starts {
            let window = &embeddings[start..start + MIN_EMBEDDINGS];
            let views: Vec<_> = window.iter().map(|e| e.view()).collect();
            let emb_sequence = ndarray::stack(Axis(0), &views)?;
            let emb_input = emb_sequence.insert_axis(Axis(0));
            for (name, session) in &mut self.classifiers {
                let tensor = Tensor::from_array(emb_input.clone())?;
                ...
                let raw = outputs["score"].try_extract_array::<f32>()?;
                let score = raw.iter().copied().next().unwrap_or(0.0);
                let slot = peak.get_mut(name).expect("zero_scores seeded every name");
                if score > *slot {
                    *slot = score;
                }
            }
    ...
    /// Resample/normalize, run the mel + embedding networks over the whole chunk, and return the
    /// full embedding sequence (76-frame windows, stride 8). The shared, expensive front-end that
    /// [`predict_peak`](Self::predict_peak) runs once per clip. Returns an empty vec when the chunk
    /// is too short to yield even one embedding.
    fn compute_embeddings(
        &mut self,
        audio_chunk: &[i16],
    ) -> Result<Vec<ndarray::Array1<f32>>, WakeWordError> {
        let samples_f32 = if self.resampler.is_some() {
            self.resample_to_16k(audio_chunk)?
        } else {
            audio_chunk.iter().map(|&x| x as f32 / 32768.0).collect()
