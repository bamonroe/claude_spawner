...
- Dedicated wake-word / end-token detector (the LiveKit epic, see `TODO.md`): `SPAWNER_WAKEWORD_URL`
  ...
  trades a few false positives for near-zero misses). `SPAWNER_WAKEWORD_TRAIN_DIR` (empty =
  disabled; base directory the in-app "add live training data" flow writes user-recorded token
  clips into, laid out `<dir>/<model>/<category>/clip_*.wav` — real-voice samples to retrain the
  detector on, complementing the synthetic training set).
