...
| `profiles`      | `{ "profiles": [{ "name", "target" }], "default": "default" }` | the execution-profile catalogue, pushed once right after `agents`: each profile's persisted `name` and advisory default `target` (`host` or `sandbox`). The built-in `default` profile is always present; older clients can ignore this and spawn with the default profile. |
