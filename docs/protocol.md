...
| `restart`       | `{}`                                      | ask the server to restart itself. The server fires `SPAWNER_RESTART_CMD` (a detached command that SSHes to the host and runs `deploy/rebuild-container.sh`, rebuilding the image and recreating the container) which relaunches the server; it broadcasts a `say` to every client, and the app auto-reconnects once the fresh process is listening. Any authenticated client may trigger this. If the command is unset, the server replies with an `error` (`restart_failed`) instead of silently doing nothing. |
...
| type            | payload                                              | meaning                                  |
|-----------------|------------------------------------------------------|------------------------------------------|
| `hello_ok`      | `server_version`, `session_id`, `whisper_model`, `whisper_model_fast`, `whisper_models` | auth accepted; `whisper_model` / `whisper_model_fast` = the accurate and fast resident whisper models (`whisper_model_fast` empty when no fast server is configured); `whisper_models` = the ggml model names on the server's disk, size-ordered (empty when `SPAWNER_WHISPER_MODELS_DIR` isn't set) — the app offers them as a picker |
...
| `whisper_model` | `{ "model": "<name>", "fast_model": "<name>", "whisper_models": ["<name>"] }` | a server-global whisper model changed: the accurate and fast servers' current models (`fast_model` empty = no fast server), plus the available model names as in `hello_ok`. Broadcast to all clients; response to `set_whisper_model`. |
