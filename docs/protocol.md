...
`history`), so no session frame leaves without it. It survives a context rotation
...
| `say`           | `{ "session_id": "...", "text": "ok bud, where do you want it?" }` | app should speak this (TTS) + display. Hub-fanned `say`s (turn breadcrumbs, the compress-completion confirmation) carry the session's `session_id`; conversational dialog `say`s sent direct to the acting client (spawn prompts, command readbacks) are for its current view and may omit it. The compress-completion `say` is a turn-terminal (buffered/redeliverable) and carries a `turn` id like the other terminals below; breadcrumb `say`s are live-only and don't. |
...
| `error`         | `{ "code": "...", "message": "..." }`                 | spoken/displayed error feedback. Turn-terminal errors (`turn_failed`, `compress_failed`, the compress `internal` failures) are hub-fanned so they carry the session's `session_id` (and the `turn` id) — they can be buffered and redelivered on reconnect, and the id lets the app drop a redelivery it already presented. Non-turn errors (`bad_message`, …) are live-only and carry no id. |
