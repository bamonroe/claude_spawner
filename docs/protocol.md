...
| `swap`          | `{}`                                      | server-side fallback for toggling back to the previously attached session — a two-way jump between the two most-recent sessions for this connection. The server tracks the previous `session_id` per connection (set on each genuine attach, and on detach); `swap` attaches to it and records the outgoing session as the new previous, so repeated swaps ping-pong. Speaks "no previous session…" when there's nothing to swap to. The voice **swap** command uses this path; the app's right-to-left chat swipe now switches its local focused session immediately and silently syncs that focus with `attach`. -> `attached` |
...
| `clear`         | `{}`                                      | rotate the attached session's Claude context: retire the current `session_id` (its transcript kept for `history`) and start a fresh one, so the next dictation replays no prior context. No model tokens spent; history still spans the whole chain. The `context_reset` carries the new `session_id`, so clients re-key to it and stop sending the retired id. -> `context_reset` + `say` |
| `compress`      | `{}`                                      | compact the attached session's Claude context: run a background turn asking Claude to summarize the conversation, then rotate the `session_id` (old transcript kept for `history`) and carry the summary forward — it seeds the next dictation so Claude continues with the condensed context instead of dropping it (the `/compact` analogue of `clear`). Emits an `activity` breadcrumb while summarizing, then a `context_reset` (carrying the new `session_id`, readout back to zero) and a `say`. Refused if a turn is in flight (`say`) or no turn has run yet (`say`). -> `activity` + `context_reset` + `say` |
...
| `host_put`      | `{ "host": { "name": "<name>", "address": "<host/ip>", "user?": "", "port?": 22, "key_file?": "", "identity?": "", "claude_bin?": "", "updated_at?": 0 } }` | add or update (upsert by `name`) an SSH host used for SSH-native execution. The app is the source of truth; the server persists it (survives restarts, shared across clients). `address` is dialed literally (NOT an `~/.ssh/config` alias). `identity` names a managed keypair (see `identities`) and, when set, supersedes `key_file`. `updated_at` is the client-stamped last-edit time in unix **milliseconds**, driving last-writer-wins: an upsert older than the stored record (or not newer than a delete tombstone) is rejected and the server re-sends its newer record to that client. **Trust-on-first-use:** the server scans the host's SSH key and records it in the server's known_hosts (so the first turn/browse isn't refused) — idempotent, so re-saving is a no-op. Errors `bad_host` if `name` is empty, or (after saving) if the host couldn't be reached to record its key. -> `host_list` broadcast to every client |
| `host_delete`   | `{ "name": "<name>", "updated_at?": 0 }`  | remove an SSH host from the registry by name; also **forgets** its known_hosts record. `updated_at` (unix ms) records a **tombstone** so a stale client can't resurrect the host with an older stamp; a delete older than the stored record is rejected (re-synced). -> `host_list` broadcast to every client |
| `identities`    | `{}`                                      | request the app-managed SSH identity registry (Settings → Identities) -> `identity_list` |
| `identity_create` | `{ "name": "<name>", "user": "<login user>", "password?": "", "gen_key?": true, "updated_at?": 0 }` | register an identity for `user` (required — the default SSH login user a host can override). `gen_key` (default true) generates a fresh keypair; `password` (optional) adds SSH password auth. A key-less identity must have a password. `updated_at` (unix ms) is the client-stamped edit time for last-writer-wins; a create not newer than a prior delete tombstone is rejected (re-synced). The server keeps the private key and password; only the public key is returned. Errors `bad_identity` on empty name/user or a duplicate. -> `identity_list` broadcast to every client |
| `identity_import` | `{ "name": "<name>", "user": "<login user>", "password?": "", "key_path": "<server path>", "updated_at?": 0 }` | register an existing server-side private key (e.g. the config default key) as a managed identity for `user`: copies it into the keys dir and records its public key. `updated_at` as for `identity_create`. Errors `bad_identity` on empty name/user/path or an unreadable/encrypted key. -> `identity_list` broadcast to every client |
| `identity_update` | `{ "name": "<name>", "user": "<login user>", "set_password?": false, "password?": "", "updated_at?": 0 }` | update an existing identity's `user`, keeping its keypair. When `set_password` is true the `password` is applied (empty clears it); otherwise the current password is kept. `updated_at` (unix ms) drives last-writer-wins: an update older than the stored record is rejected (re-synced). Errors `bad_identity` on empty name/user, unknown identity, or leaving a key-less identity with no password. -> `identity_list` broadcast to every client |
| `identity_delete` | `{ "name": "<name>", "updated_at?": 0 }` | remove an identity and its private key by name. `updated_at` (unix ms) records a **tombstone** blocking a stale re-create; a delete older than the stored record is rejected. -> `identity_list` broadcast to every client |
| `profile_put`   | `{ "profile_def": { "name": "<name>", "target?": "host\|sandbox", "default?": false, "image?": "", "home_mount?": "", "mounts?": [], "creds?": [], "env?": {}, "run_args?": [], "vars?": {}, "updated_at?": 0 } }` | add or update (upsert by `name`) an execution profile. The app is the source of truth; the server persists the catalogue (survives restarts, shared across clients). String fields are `{{.Var}}`-templated per turn. `updated_at` (unix ms) drives last-writer-wins as for `host_put`. Errors `bad_profile` on an empty name or an invalid env key. -> `profiles` broadcast to every client |
...
| `spoken_token_put` | `{ "spoken_token": { "name": "<name>", "phrase": "<spoken words>", "action": "wake\|end\|speech_gate", "model?": "", "updated_at?": 0 } }` | add or update (upsert by `name`) a spoken token in the app-managed catalogue (Settings → Spoken tokens). `phrase` is matched whole-word, case- and punctuation-insensitive; `action` must be one of the advertised `actions`; `model` is the optional dedicated-detector (ONNX) key scored when the wakeword sidecar is on (empty = Whisper string-match). The configured tokens fully REPLACE the built-in "hey buddy"/"beep" defaults. `updated_at` (unix ms) drives last-writer-wins as for `host_put`. Errors `bad_token` on an empty name or unknown action. -> `spoken_tokens` broadcast to every client |
| `spoken_token_delete` | `{ "name": "<name>", "updated_at?": 0 }` | remove a spoken token by name. `updated_at` (unix ms) records a **tombstone** so a stale client can't resurrect it; a delete older than the stored token is rejected. -> `spoken_tokens` broadcast to every client |
...
detection falls back to the Whisper string-match), `denoise`/`denoise_atten_db` (server-side
denoise: when `denoise` is true and the server advertised it via `hello_ok` `denoise`
(`SPAWNER_DENOISE_URL` set), each clip is scrubbed of steady background noise by the DeepFilterNet
sidecar before transcribing; `denoise_atten_db` caps the attenuation in dB — DeepFilterNet's
`atten_lim_db`, lower is gentler, `<=0` = full. Off by default; adds latency; any sidecar failure
falls back to the original clip), `aliases` (misheard→command
...
set via `setting_put`) the same way, and a sixth, `spoken_tokens_digest`, covers the **spoken-token**
catalogue (the wake/end/speak phrases set via `spoken_token_put`/`spoken_token_delete`). The `warm_compress`/`auto_compress`/`auto_compress_threshold`
...
Every **session-scoped** frame below carries a stable `session_id` — the session
the frame belongs to — so a client viewing a *different* session routes it to the
right chat log instead of misfiling it under whatever it currently shows. The id
is authoritative and rename-stable (a `name` alongside it is display-only, and can
diverge); it is the same id the session is keyed by everywhere else. The server
stamps it at one choke point (the per-connection job sink every fan-out passes
through) plus the two direct responses (`transcript` to the capturing client,
`history`); and any `say`/`error` sent **directly** from the read loop (dialog
prompts, session-op readbacks, transcription failures) is stamped at the send
...
| type            | payload                                              | meaning                                  |
|-----------------|------------------------------------------------------|------------------------------------------|
| `hello_ok`      | `server_version`, `session_id`, `whisper_model`, `whisper_model_fast`, `whisper_models`, `whisper_models_local`, `tts`, `denoise` | auth accepted; `tts` = the server offers Kokoro speech synthesis (`SPAWNER_TTS_URL` set), so the client may send `speak` instead of using on-device TTS; `denoise` = the server offers server-side noise removal (`SPAWNER_DENOISE_URL` set), so the client's `hello` `denoise` toggle takes effect; `whisper_model` / `whisper_model_fast` = the accurate and fast resident whisper models (`whisper_model_fast` empty when no fast server is configured); `whisper_models` = the curated English-model catalogue offered as a picker (plus any extra ggml file on disk), and `whisper_models_local` = the subset already downloaded — an entry in `whisper_models` but not `whisper_models_local` is fetched on select. Both empty when `SPAWNER_WHISPER_MODELS_DIR` isn't set (free-text entry) |
| `agents`        | `{ "agents": [{ "id", "name", "default_model", "models": [{ "alias", "voice" }], "updated_at" }], "default": "claude" }` | the AI backend registry, pushed once right after `hello_ok` and re-broadcast after any `provider_put`: each backend's id, display name, *effective* default model alias (the Providers-tab override, else the compiled default), and selectable models. Each model carries `voice` — whether the voice `list models`/`use model N` commands enumerate it (toggled per model in the Providers tab). `updated_at` (unix ms) is the stored override's last-edit stamp (0 = no override) so the app can arbitrate last-writer-wins. `default` is the backend a spawn gets when none is chosen. The app uses it to offer a backend + model choice in the new-session picker (sent in `spawn_at`), to label the model catalogue, and to render Settings → Providers. |
| `profiles`      | `{ "profiles": [{ "name", "target", "default", "image", "home_mount", "mounts", "creds", "env", "run_args", "vars", "updated_at" }], "default": "<name>" }` | the app-managed execution-profile catalogue (Settings → Profiles), pushed once right after `agents` and broadcast after any `profile_put`/`profile_delete`/`profile_set_default`. Each entry is the full profile so the editor can round-trip it; omitted fields are empty. `updated_at` (unix ms) is each profile's last-edit stamp for last-writer-wins. `default` (top level) names the marked-default profile a no-choice session resolves to (empty catalogue → `""`); a profile's own `default` flag marks it. Default-first order. Older clients read only `name`/`target`. |
| `actions`       | `{ "actions": [{ "id": "wake\|end\|speech_gate", "label": "<name>", "desc": "<one-line>" }] }` | the closed, compile-time set of bindable spoken-token actions, advertised on connect (right after `profiles`) so the app's token editor can offer an action dropdown. Advertise-only — the app never echoes it. Never changes at runtime. |
| `spoken_tokens` | `{ "spoken_tokens": [{ "name", "phrase", "action", "model", "updated_at" }] }` | the app-managed spoken-token catalogue (Settings → Spoken tokens), pushed once on connect when its `spoken_tokens_digest` differs and broadcast after any `spoken_token_put`/`spoken_token_delete`. Each entry is the full token so the editor can round-trip it; `model` is empty for Whisper-only tokens. `updated_at` (unix ms) is each token's last-edit stamp for last-writer-wins. |
...
| `transcript`    | `{ "session_id": "...", "name": "<session>", "text": "...", "final": true }` | STT result (may stream partials), echoed as the user's chat bubble. `session_id` is the session the clip was captured for so the app files the bubble under it rather than whatever session is attached when this (async) frame lands — switching sessions mid-transcription can't misattribute it (`name` is display-only). Empty/omitted (detached, or an older server) → the app keys it to the current view. |
...
| `activity`      | `{ "session_id": "...", "text": "🤔 thinking…" }`    | what Claude is doing right now during a turn (thinking / running a tool / editing a file); transient status line, not spoken |
...
| `files`         | `{ "session_id": "...", "files": ["a.go", "b.md"] }` | basenames of files changed so far this turn; a persistent "edited: …" chip |
...
| `say`           | `{ "session_id": "...", "text": "ok bud, where do you want it?" }` | app should speak this (TTS) + display. Hub-fanned `say`s (turn breadcrumbs, the compress-completion confirmation) carry the session's `session_id`; conversational dialog `say`s sent direct to the acting client (spawn prompts, command readbacks) are stamped at the send seam with the connection's **attached** session, so they too file under the right chat log — only a `say` sent while the connection is **detached** (e.g. "picking up where we left off", or a spawn dialog before any attach) omits it. The compress-completion `say` is a turn-terminal (buffered/redeliverable) and carries a `turn` id like the other terminals below; breadcrumb `say`s are live-only and don't. |
...
| `context_reset` | `{ "name": "...", "session_id": "..." }`              | the session's Claude context was rotated to a fresh one — a `clear` (empty) or a `compress` (seeded with a summary). The app drops its last-turn token accounting so the status-bar context-size readout returns to zero; nothing shows again until the next dictation runs against the new context (which reports the true new size). Carries the fresh `session_id` (the old one retired onto the session's prior-id chain), so this reset is a single self-describing event: the app re-keys to it and refreshes the cleared session's locally-cached rows off it, rather than inferring the change or reading it from a re-attach. Emitted for `clear`/`compress` and their voice commands. |
| `renamed`       | `{ "old": "...", "name": "...", "session_id": "<uuid>" }` | the currently-attached session was renamed (from the sidebar `rename_discovered` or the `rename` command). Carries the `old` and new `name` plus the stable `session_id` — the app matches by id (names diverge between servers) and updates the attached-session **title in place** — no history refetch or context-meter reseed (unlike a fresh `attached`). Sent to **every** connection attached to the renamed session — the initiator plus any other device the user has on the same session — so each updates its title in place instead of inferring the rename from a later `discovered`/session-list diff. Connections attached to another session, or none, don't receive it. |
| `output`        | `{ "session_id": "...", "name": "...", "text": "...", "chunk": true, "turn": "..." }` | clean session output (for display + TTS). `turn` is an opaque id shared by every frame of one turn (all `chunk: true` messages **and** the closing `chunk: false`) — the client's dedup key: key "did I already show/speak this turn" on it, never on text equality (a backend may reshape the closing text vs the streamed chunks, and a close can reach a client twice via buffered redelivery). Claude's prose **streams live**: one `chunk: true` message per assistant text message as it lands, then a final `chunk: false` closing the turn. A client that saw the stream shows/speaks the chunks and treats the final as an end marker; a client that missed it (a reply buffered because nobody was attached, or because the client was briefly unreachable — backgrounded / a mobile stall — so the send failed) gets only the final `chunk: false` and renders that. Such a buffered-undelivered reply is **not** discarded by the next turn: the server redelivers it the moment the socket accepts a write again (at the next turn's start) or when the client reattaches, so a reply that landed during a stall still reaches the app. The final `chunk: false` message also carries a `usage` object — `{ "input", "output", "cache_write", "cache_read" }` token counts for the turn (from the stream-json `result` event's aggregate `usage`) — and `usage_at`, this turn's completion time in unix seconds. `cache_read > 0` means a warm prompt-cache hit; `cache_write > 0` means the cache was (re)built. The app renders this as a per-message token badge and drives its cache-warm indicator, anchoring the cache-warm countdown to `usage_at` so a reply delivered buffered on reconnect still counts down from the turn's real age rather than from when it arrived. The closing frame additionally carries `turns` (int) and `turn_total` (same `{ "input", "output", "cache_write", "cache_read" }` shape as `usage`): a single dictation drives an **agentic loop** of several model calls (each tool round-trip is its own API request/response cycle), so `turns` is how many cycles ran and `turn_total` is the `result` event's aggregate usage summed across all of them — distinct from `usage`, which is the current context-window snapshot. The app shows both in the **detailed** token badge so the aggregate reads in context (N cycles, not one). Streaming `chunk: true` messages omit all of these (no per-chunk accounting). |
| `history`       | `{ "session_id": "...", "name": "...", "messages": [{ "index", "id"?: "...", "role": "user"\|"claude", "text", "ts": <unix s>, "usage"?: {…}, "turns"?: <int>, "turn_total"?: {…} }], "more": true, "count": <int>, "hash": "<hex>", "unchanged": false }` | a page of past conversation (oldest→newest); `more` = older messages remain to page in. `ts` is the message's transcript timestamp in unix seconds (0 if the transcript line lacked one). `id` is the row's **durable** identity read from the backend transcript (Claude JSONL `uuid`, opencode `msg_…` id; omitted for Codex/Antigravity, which expose none yet). On the closing `claude` row of a dictation, `usage` is that turn's context snapshot and `turns`/`turn_total` are the same agentic-loop rollup the live closing `output` frame carries — reconstructed from the transcript (every assistant line since the last user message, tool-only ones included), so the detailed badge keeps its "N turns · X total" tail after a reattach. Omitted where zero — unlike `index` (a positional cursor rewritten on every clear/compress rotation and cross-backend re-index), `id` is stable across rotations, so the app reconciles a live row against its history row by `id` when present and only falls back to `index`/text when absent. A `claude` message carries `usage` (same `{ "input", "output", "cache_write", "cache_read" }` shape as `output`) so the per-message token badge survives a reattach or server restart — read from the transcript and set only on the **final** assistant line of a turn (matching the live badge, which lands on the closing message); omitted otherwise. `user` messages have the server-injected prompt scaffolding (the brief-reply nudge, interactive-mode ask instruction, compress recap preamble) stripped, so replayed history matches the live echo exactly and the app can dedupe a turn against its live copy. `count`/`hash` are the whole chain's digest — the app stores them with its cached transcript and later sends `hash` back as `have_hash` to check freshness. `unchanged: true` answers a top-page request whose `have_hash` still matched: `messages` is empty and the app keeps its cache untouched. Response to `history`. |
...
| `host_list`     | `{ "hosts": [{ "name", "address", "user?", "port?", "key_file?", "identity?", "claude_bin?", "updated_at?" }] }` | the app-managed SSH host registry (Settings → Hosts). Response to `hosts`, and broadcast to every client after a `host_put`/`host_delete` so the shared list stays in sync. Each host carries `updated_at` (unix ms, 0 for a pre-timestamp record) for the app's last-writer-wins merge. |
| `identity_list` | `{ "identities": [{ "name", "user", "public_key", "has_password", "updated_at" }] }` | the app-managed SSH identity registry (Settings → Identities) — name, default login `user`, PUBLIC key (empty for a password-only identity), whether a password is set, and `updated_at` (unix ms) for last-writer-wins. The private key and the password itself never leave the server. Response to `identities`, and broadcast after `identity_create`/`identity_import`/`identity_delete`. |
| `error`         | `{ "code": "...", "message": "..." }`                 | spoken/displayed error feedback. Turn-terminal errors (`turn_failed`, `compress_failed`, the compress `internal` failures) are hub-fanned so they carry the session's `session_id` (and the `turn` id) — they can be buffered and redelivered on reconnect, and the id lets the app drop a redelivery it already presented. A direct `error` (e.g. a transcription failure) is stamped at the send seam with the connection's **attached** session, so it files under the right chat log; only errors raised while **detached** (`unauthorized`, `bad_message`, … before any attach) carry no id. |
| `turn_interrupted` | `{ "session_id": "...", "name": "...", "reason": "server restarting" }` | an in-flight dictation turn was abandoned server-side (turns don't survive a server restart). The app clears its "thinking…" state and prompts the user to resend, instead of waiting on a reply that will never arrive. |
| `turn_stopped`  | `{ "session_id": "...", "name": "...", "turn": "..." }` | a running turn was deliberately aborted (the `abort` message / "stop the turn" command). The app clears its "thinking…" state without the "say it again" nudge. Turn-terminal: carries the turn's `turn` id so a buffered redelivery on reconnect is dropped, not re-presented. |
| `diff`          | `{ "session_id": "...", "text": "..." }`              | a compact `git diff --stat` review summary after a turn that changed files; shown as a note, not spoken. |
| `ask`           | `{ "session_id": "...", "name": "...", "turn": "...", "questions": [{ "q": "...", "options": ["...", ...] }] }` | interactive mode: Claude needs clarification. The app renders the questions (chips for `options`, text fields otherwise) and reads them aloud; answers go back as an ordinary dictation turn. `options` omitted = free-text. Turn-terminal: an `ask` ends its turn in place of the closing `output`, can be buffered and redelivered on reconnect, and carries the turn's `turn` id — the app must key "did I already present these questions" on it, or a reconnect re-renders and re-speaks them. |
...
(the last assistant message). Every frame of one turn — chunks and close — carries the same opaque
`turn` id, and that id is the **only** reliable dedup key: clients dedupe by tracking whether they
saw any `chunk: true` with the close's `turn` id. If so, the chunks were already shown/spoken and
the `chunk: false` is just an end-of-turn marker; if not (the turn finished while the client was
detached and its reply was buffered for reconnect), the client renders/speaks the `chunk: false`
text. Do **not** dedupe chunk-vs-close by comparing text: some backends reshape the closing text
(Antigravity restores paragraph breaks; opencode joins per-part chunks; Claude/Codex's close is
only the *last* assistant message of a multi-message turn), so text equality is not guaranteed. A
close with a `turn` id the client already closed is a redelivery — drop it. The interactive-mode
...
`bad_delete`, `bad_rename`, `bad_host`, `bad_identity`, `bad_profile`, `bad_token`, `bad_provider`, `bad_setting`, `unauthorized`, `internal` — stay screen-only (no spoken line).
...
| `bad_token`        | invalid `spoken_token_put`/`spoken_token_delete` (missing name or unknown action) |
