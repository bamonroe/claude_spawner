...
| `spawn_at`      | `{ "path": "<dir>", "target": "host\|sandbox", "create": false }` | open `path`: if it already has a registered session, attach to that one (no duplicate `-2` is minted); otherwise create a session there and attach -> `attached` (+ `session_list` when a new one was created). `target` optional (default `host`); `sandbox` runs turns in an isolated container and errors if no sandbox image is configured. `create` optional (default `false`): when `true`, `mkdir` the (jailed) `path` first so the picker can start a project in a directory that doesn't exist yet — errors `bad_path` if the folder already exists or is outside the roots |
...
| `restart`       | `{}`                                      | ask the server to rebuild and relaunch itself. The server runs as a Docker container and can't rebuild its own image, so it asks the host-side broker to run `SPAWNER_BROKER_RESTART_CMD` (a `docker compose … up -d --build`), which recreates the container on current code; it broadcasts a `say` to every client, and the app auto-reconnects once the fresh process is listening. Any authenticated client may trigger this. If there's no broker or the command is unset, the server replies with an `error` (`restart_failed`) instead of silently doing nothing. |
