...
| `adopt`         | `{ "session_id": "<uuid>", "path": "<dir>" }` | register a discovered session into the store and attach to it (so the app can view/drive it via `--resume`) -> `attached` + `session_list`. If the folder already has a live local session (the given `session_id` is stale — e.g. a cached row tapped on a fresh offline open), attaches to that existing session instead of registering a phantom `<dir>-2` duplicate |
...
| `restart`       | `{ "rebuild?": true }`                    | ask the server to restart itself. The server fires `SPAWNER_RESTART_CMD` (a detached command that SSHes to the host and runs `deploy/rebuild-container.sh`, rebuilding the image and recreating the container) which relaunches the server; it broadcasts a `say` to every client, and the app auto-reconnects once the fresh process is listening. `rebuild` (default true when absent, for older clients and the voice command) recompiles from source; `false` requests a fast **bounce** that recreates from the existing image without recompiling — the server passes the choice to `rebuild-container.sh` by substituting the `%REBUILD%` token in `SPAWNER_RESTART_CMD` with `rebuild`/`bounce`. Any authenticated client may trigger this. If the command is unset, the server replies with an `error` (`restart_failed`) instead of silently doing nothing. |
| `speak`         | `{ "id": "<client id>", "text": "...", "voice?": "" }` | ask the server to synthesize `text` (markdown already stripped client-side) via the resident Kokoro TTS server. `id` is a client-chosen correlation id echoed on the response stream; `voice` (optional) overrides the server default (`SPAWNER_TTS_VOICE`). The speak/mute/summary-only decision stays client-local — nothing is synthesized for a client that doesn't ask. Refused with an immediate error-bearing `speak_end` when server TTS is disabled, `text` is blank, or the connection's speak queue (32) is full — the client falls back to on-device TTS. -> `speak_audio` + binary frames + `speak_end` |
...
| `speak_audio`   | `{ "id": "<client id>", "codec": "opus" }`           | heads one synthesized utterance (response to `speak`): the **binary frames** that follow, up to the matching `speak_end`, are its audio in `codec` (the server's `SPAWNER_TTS_FORMAT`: `opus` \| `mp3` \| `wav` \| `flac` \| `pcm`). This is the only server→client binary; speaks are serviced **one at a time per connection, in request order**, so the frames between a header and its end always belong to that `id` — streams never interleave |
| `speak_end`     | `{ "id": "<client id>", "error": "" }`               | closes a speak stream. `error` empty = success; non-empty (synthesis failed, tts disabled, empty text, queue full — possibly with no `speak_audio` ever sent) means the client should fall back to on-device TTS for that utterance |
...
| `discovered`    | `{ "sessions": [{ "name", "dir", "session_id", "last_active": <unix s>, "active": <bool>, "registered": <bool>, "busy": <bool>, "target?", "host?", "agent?", "model?" }] }` | the session list: **every registered session as its own row** (keyed by its `session_id`, so multiple sessions in one directory are each shown and separately manageable), plus one adoptable row per **unregistered** directory found on disk. `active` = an interactive `claude` is open in tmux at that dir (driving it then risks a two-writer conflict); `registered` = already in the store; `busy` = a dictation turn is running for it now; `target` present only for non-host sessions (`"sandbox"`), so the app can badge them. `host` present only for sessions running on a registered SSH host (its host name; omitted = local), so the app can group them. `agent`/`model` (registered rows only; `agent` omitted for the default Claude) name the AI backend + current model so the app can badge them. Response to `discover`. |
