...
| type            | payload                                              | meaning                                  |
|-----------------|------------------------------------------------------|------------------------------------------|
| `hello_ok`      | `server_version`, `session_id`, `whisper_model`, `whisper_model_fast` | auth accepted; `whisper_model` / `whisper_model_fast` = the accurate and fast resident whisper models (`whisper_model_fast` empty when no fast server is configured) |
...
| `whisper_model` | `{ "model": "<name>", "fast_model": "<name>" }`      | a server-global whisper model changed: the accurate and fast servers' current models (`fast_model` empty = no fast server). Broadcast to all clients; response to `set_whisper_model`. |
