...
| `context_reset` | `{ "name": "..." }`                                   | the session's Claude context was rotated to a fresh one — a `clear` (empty) or a `compress` (seeded with a summary). The app drops its last-turn token accounting so the status-bar context-size readout returns to zero; nothing shows again until the next dictation runs against the new context (which reports the true new size). The rotation mints a fresh `session_id`, but it rides on the refreshed `attached` emitted alongside this reset (see `clear`/`compress`), not here — the app re-keys and refreshes the cleared session's rows off that. Emitted for `clear`/`compress` and their voice commands. |
