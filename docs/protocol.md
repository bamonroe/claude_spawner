...
| `spawn_at`      | `{ "path": "<dir>", "target": "host\|sandbox", "create": false, "host_name": "" }` | open `path`: if it already has a registered session, attach to that one (no duplicate `-2` is minted); otherwise create a session there and attach -> `attached` (+ `session_list` when a new one was created). `target` optional (default `host`); `sandbox` runs turns in an isolated container and errors if no sandbox image is configured. `create` optional (default `false`): when `true`, `mkdir` the (jailed) `path` first so the picker can start a project in a directory that doesn't exist yet — errors `bad_path` if the folder already exists or is outside the roots. `host_name` optional (default `""` = local): the registered SSH host (Settings → Hosts) to run the session on; ignored for `sandbox` |
...
| `identity_list` | `{ "identities": [{ "name", "public_key" }] }` | the app-managed SSH identity registry (Settings → Identities) — names and PUBLIC keys only (private keys never leave the server). Response to `identities`, and broadcast after `identity_create`/`identity_delete`. |
