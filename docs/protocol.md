...
| `history`       | `{ "name": "...", "messages": [{ "index", "role": "user"\|"claude", "text", "ts": <unix s> }], "more": true }` | a page of past conversation (oldest→newest); `more` = older messages remain to page in. `ts` is the message's transcript timestamp in unix seconds (0 if the transcript line lacked one). `user` messages have the server-injected prompt scaffolding (the brief-reply nudge, interactive-mode ask instruction, compress recap preamble) stripped, so replayed history matches the live echo exactly and the app can dedupe a turn against its live copy. Response to `history`. |
