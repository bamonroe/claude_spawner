...
| `context_reset` | `{ "name": "..." }`                                   | the session's Claude context was rotated to a fresh, empty one (a `clear`). The app drops its last-turn token accounting so the status-bar context-size readout returns to zero; nothing shows again until the next dictation runs against the new context. Emitted in response to `clear` (and the "clear" voice command). |
