...
| `transcript`    | `{ "name": "<session>", "text": "...", "final": true }` | STT result (may stream partials), echoed as the user's chat bubble. `name` is the session the clip was captured for so the app files the bubble under it rather than whatever session is attached when this (async) frame lands — switching sessions mid-transcription can't misattribute it. Empty/omitted (detached, or an older server) → the app keys it to the current view. |
...
| `say`           | `{ "text": "ok bud, where do you want it?" }`        | app should speak this (TTS) + display. The compress-completion `say` is a turn-terminal (buffered/redeliverable) and carries a `turn` id like the other terminals below; breadcrumb `say`s are live-only and don't. |
...
| `error`         | `{ "code": "...", "message": "..." }`                 | spoken/displayed error feedback. Turn-terminal errors (`turn_failed`, `compress_failed`, the compress `internal` failures) additionally carry the `turn` id — they can be buffered and redelivered on reconnect, and the id lets the app drop a redelivery it already presented. Non-turn errors (`bad_message`, …) are live-only and carry no id. |
