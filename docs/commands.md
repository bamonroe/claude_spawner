...
|-----------------|------------------------------------------|---------------------------------------------------|
| `spawn`         | "spawn a new session" / "new session called bugfix in data" / "new session in git personal on codex with sandbox profile" / "spawn a new project in git personal" | Starts a session. Everything after the noun is optional; anything you don't say uses a default. **Name** — "called <name>" / "named <name>" (default: the folder basename). **Location** — after "in"/"at"/"under" (default: the **`directory`** setting if one is set, else the dialog asks). **Provider** — "on codex" / "on ollama" / "on zen" / "on pickle" / "a codex session" (default: Claude; old "on opencode" maps to Ollama); the session is stamped with that backend and its default model. **Profile** — "with <name> profile" / "profile <name>" (default: the marked-default execution profile). When the location resolves to a concrete folder (or is omitted → home), the server **skips the dialog and creates + attaches immediately** with those defaults. It falls back to the interactive dialog (below) for "new project" (folder creation), a location it can't resolve or only fuzzily matches, or one that lands on a root/namespace with sub-folders to choose among. |
| `attach`        | "attach to `<name>`"                     | Attaches; subsequent speech is dictated           |
| `detach`        | "detach" / "stop dictating"              | Leaves passthrough mode                            |
| `swap`          | "swap" / "swap back" / "previous session" / "last session" | Jumps back to the session you were attached to **just before** this one — a two-way toggle, so saying it again returns you. The server remembers the previous session per connection (set whenever you attach elsewhere, and on detach), so no name is needed. The app's **right-to-left swipe** on the chat screen does the same jump locally first, then silently syncs focus to the server. Speaks "no previous session…" if you haven't been anywhere else yet, or "the previous session is gone" if it was killed meanwhile. |
...
| `set`           | "set target `<host>`" / "set the target to `<host>`" / "set directory `<path>`" / "set folder to `<path>`" | Sets one of the two **where** settings: **`target`** (also "host"/"machine") — the host that new sessions spawn on *and* that shell commands run on; **`directory`** (also "folder"/"dir"/"path") — the directory a spawn with no spoken path lands in. Both are validated before they're stored: a target must name a configured host (fuzzily matched, so "bahm" finds `bam`; otherwise the known hosts are read back), and a directory is walked segment-by-segment against the **target's real filesystem** (same fuzzy path resolution as the spawn dialog) and stored as the resolved absolute path. Values persist in the shared-settings catalogue, so they survive a server restart and sync to every client. "set model `<n>`" remains `use_model`, not a setting. |
| `get`           | "get target" / "what is the target" / "what's the directory" / "where is the directory" | Speaks the current value. An unset target reads back as `localhost` (the fallback); an unset directory says a path will be asked for on spawn. Deliberately narrow — the key must be the *whole* rest of the utterance, so "what is the target of this refactor" is dictation. |
| `auth_status`   | "auth status" / "am I logged in" / "claude login status" | Speaks whether the **target host's** Claude is logged in and as whom (account, org, plan), and pushes the same `auth_status` message every client shows in its Hosts row. Read-only; never starts a login. |
| `auth_login`    | "log in to claude" / "log claude in" / "re-login to claude" | Starts the Claude re-login on the **target host**, reusing the billing identity that host already had. The spoken reply only says the login has started and to check the app — the OAuth URL is unusable as speech, so it arrives as the usual `auth_login_url` message and opens the app's login panel. Refused (spoken) if SSH is disabled. |
...
bam slash git on zen"). A **bare** "new session" (no path), or one whose path is ambiguous or
